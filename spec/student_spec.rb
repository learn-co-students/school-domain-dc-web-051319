...
describe 'School' do
  ...
  describe "#sort" do
    it 'is able to sort the students' do
      ...
      @school.add_student("Jack Baur", 7)

      # key order does not matter; this is testing that the students in each respective value are in alphabetical order
      expect(@school.sort).to eq({7 => ['Blake Johnson', 'Jack Baur'], 9 => ["Homer Simpson", "Bart Simpson"], 10 => ["AC Slater", "Jeff Baird", "Blake Johnson", "Avi Flombaum"]})
    end
  end
end
